...
class MessageBuilder {
    ...
    buildMediaReply(reply, addr, context) {
      ...
      return new builder.Message().address(addr)
    ...
    buildCarouselReply(reply, addr, context) {
      ...
      for (let i = 0; i < reply.content.length; i++){
        currentCard = new builder.HeroCard()
                          ...
                          .images([
                              builder.CardImage.create(null, reply.content[i].image)
                          ])
                          .buttons([
                              builder.CardAction.openUrl(null, reply.content[i].buttonLink, reply.content[i].buttonText)
                          ]);
