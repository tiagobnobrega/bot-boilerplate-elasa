...
const laisClient = function (initArgs) {
    ...
    let loadArgs = function loadArgs(){
        ...
        if(!endPointUrl){
            throw new Error("Não foi possível determinar o valor para url. Por favor verifique a variável de ambiente LAIS_CORE_FALA_API_URL ou os parâmetros passados para o cliente.");
        }
        if(!connectorId){
            throw new Error("Não foi possível determinar o valor para url. Por favor verifique a variável de ambiente LAIS_CORE_CONNECTOR_ID ou os parâmetros passados para o cliente.");
        }
    };
