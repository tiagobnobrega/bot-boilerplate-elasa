...
class apiConfig {
    ...
    static allHeadersValidation(route,headers){
        ...
        this.routesHeaderDependecies[route].forEach((currentHeader)=>{
            ...
            let spacer = ( feedback.message == "" ? '' : ' ; ' ),
                hasHeaderErrorMessage = this.headerErrorMessage(this.headersDependenciesValidValues,headers,currentHeader,spacer);
    ...
    static headerErrorMessage(headersDependenciesValidValues,headers,header,spacer) {
        let reqCondition = headersDependenciesValidValues[header] === headers[header];
        return (reqCondition ? false : `${spacer}header ${header} inválido ou inexistente`);
    }

    static nextRequestAction(headersValidationFeedback,next){
        return (headersValidationFeedback.errors > 0 ? next(new errs.UnauthorizedError({"code": "401", "message": headersValidationFeedback.message})) : next());    
    }
}
