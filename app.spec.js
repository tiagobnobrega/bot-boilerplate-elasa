...
const postFileMessage = async ({action, messages}) => {
    return postMessage({type: 'file', payload: {action, messages}})
};
...
describe('Testes integrados do bot', () => {
    ...
    test('mensagem tipo "file" no contexto raiz com erros', async () => {
        const postedFile = {
        ...
        const {replies} = await postFileMessage(postedFile);
        expect(replies).toHaveLength(1);
    ...
    test('mensagem tipo "file" no contexto raiz SEM erros', async () => {
        const postedFile = {
            "action": "normal_price.file_validation",
            "messages": [
                {
